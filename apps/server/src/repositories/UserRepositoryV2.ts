...
export class UserRepository extends BaseRepository<User, PrismaUser> implements IUserRepository {
    ...
    // Override create to handle identityId for user creation
    async createWithIdentity(data: Omit<User, 'id' | 'createdAt' | 'updatedAt'> & { identityId: string }): Promise<User> {
        try {
            ...
            this.handleError(error, 'createWithIdentity', { data });
            throw error;
        }
    }
}
